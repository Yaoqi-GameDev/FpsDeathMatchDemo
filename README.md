...
| 2026-04-18 | **客户端移动预测（最小）**：**`PlayerLocomotionNetBridge`** 发输入带 **`ClientTick`**；服务器 **`NetworkVariable`** 同步权威位置/速度/tick；纯客户端 Owner 关闭 **`NetworkTransform`**、本地跑 **`FpsPlayerMotor`**；校正分级：忽略 / 软插值 / 套权威运动学 / 超大误差 **`ApplyAuthoritativeHardResync`** |
