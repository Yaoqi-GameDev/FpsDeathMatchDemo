...
- 全局入口：`UIFrameService.Ensure` → 一份 UIFrame（优先挂到唯一 **`--DDOL--`**）；`ShowLobbyMenu` 回大厅强制开大厅并复位 alpha。
- **`--DDOL--`** 全局只留一个（重复根合并子物体后自毁）；NetworkManager 常驻 + `LobbyNetworkManagerGuard` 去重；Return 只 `Shutdown` 不 Destroy NM。
- EventSystem：只保留 UIFrame 上那份，进 Lobby/对局会关掉多余的。
- Lobby 启动：`LobbyUiBootstrap` → `UIFrameService.ShowLobbyMenu()`。
...
- ScreenId：`LobbyMenuWindowController`、`SettingsWindowController`、`InMatchPauseMenuWindowController`（局内 ESC）、`EndGameWindowController`、`DeathmatchHudPanelController`、`HurtOverlayPanelController`（Prioritary）。
- 局内 **ESC**：暂停窗（Resume / Settings / Return to Lobby）；Settings 与大厅共用同一 Screen。
...
| 日期 | 说明 |
|------|------|
| 2026-08-01 | **DDOL 收口**：唯一 `--DDOL--`；`ShowLobbyMenu` 修二次回大厅 UI 消失；NM Guard + Return 不 Destroy；单 EventSystem |
| 2026-08-01 | **局内 ESC 暂停**：`InMatchPauseMenuWindowController` + `LobbyNetSession.TryReturnToLobby`；Settings 与大厅共用 |
...
| 2026-04-18 | **再来一局（Again）**：联机时 **`MatchManager.RestartMatch`** → **`NetMatchManager.RequestRestartMatch`**，服务器 **`NetworkManager.SceneManager.LoadScene(DeathMatch)`**；客户端用 **`ServerRpc`** 请求主机加载（避免仅用 **`SceneManager.LoadScene`** 导致 NGO 不同步、房主按钮无效） |
...
| 2026-04-18 | **客户端移动预测（最小）**：**`PlayerLocomotionNetBridge`** 发输入带 **`ClientTick`**；服务器 **`NetworkVariable`** 同步权威位置/速度/**身体 Yaw**/tick；纯客户端 Owner 关闭 **`NetworkTransform`**、本地跑 **`FpsPlayerMotor`**；校正分级：忽略 / 软插值 / 套权威运动学 / 超大误差 **`ApplyAuthoritativeHardResync`**；**Yaw** 与位置同级软跟，位置在忽略档时仍可单独 **`ApplyAuthoritativeBodyYaw`**，避免横移与朝向不一致 |
| 2026-04-18 | **输入历史 + 和解重放**：**`ClientLocomotionInputHistory`**（与 Bridge 同物体）环形保存每 tick 的 **`PlayerLocomotionInput`** 与当时 **`Time.deltaTime`**；**`LateUpdate`** 若 **`lastTick > serverTick`** 且区间完整：先 **`ApplyAuthoritativeKinematics`** 再按 tick 顺序 **`FpsPlayerMotor.SimulationStep`**；否则回退上述分级校正；**`lastTick < serverTick`** 时 **`ApplyAuthoritativeHardResync`** |
